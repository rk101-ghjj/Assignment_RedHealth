...
export default function AddCommentForm({ onAdd }: Props) {
  ...
  const selectedUser = useMemo(() => users.find((u: User) => u.id === userId), [userId]);
      ...
      {!isDropdownOpen && (
        <div className="mt-5">
          <button
            type="submit"
            disabled={isDisabled}
            className="inline-flex items-center rounded-md bg-[#5B8EFF] hover:bg-[#4E7FF0] text-white px-4 py-2 text-sm font-medium shadow-sm disabled:opacity-60 disabled:cursor-not-allowed w-full sm:w-auto gap-2"
          >
          {SEND_ICON}
          Post Comment
          </button>
