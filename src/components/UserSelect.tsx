...
export default function UserSelect({ value, onChange, placeholder = "Choose a user to comment as...", onOpenChange }: Props) {
  ...
  const selected = useMemo(() => allUsers.find((u: User) => u.id === value), [value]);
    ...
    <div className="relative" ref={containerRef}>
      ...
      >
        {selected ? (
          ...
          <span className="pointer-events-none truncate text-slate-400">{placeholder}</span>
        )}
        <svg
                ...
                <li key={u.id}>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 truncate">
                      <span className="font-medium text-slate-900 group-hover:text-white">{u.name}</span>
                      <span className="text-slate-500 group-hover:text-white"> ({u.organization})</span>
                    </div>
                  </button>
                </li>
              );
